...
public class SubmissionController {
    ...
    @GetMapping("/{matchId}")
    ResponseEntity<?> getSubmissionsOfMatch(@PathVariable UUID matchId) {
        return ResponseEntity.ok(
                submissionRepository.findByMatchIdAndStatusNotIn(matchId, List.of(Submission.Status.PROCESSING, Submission.Status.IN_QUEUE))
                        .stream().map((sub) -> new SubmissionResponseDTO(
                                sub.getId(),
                                sub.getProblemId(),
                                sub.getLanguageId(),
                                sub.getStatus().name(),
